...
* **SQLAlchemy (create\_engine):** Establishing connections and interacting with PostgreSQL.
* **python-dotenv (load\_dotenv):** Secure management of environment variables.
...
To provide real-time metrics and streaming analytics, we integrate **Apache Kafka** into our Docker environment, along with two additional scripts housed in the `scripts/` folder:

* **consumer\_service.py:** A FastAPI-based Kafka consumer that reads survey data from the Kafka topic and exposes it via REST endpoints.
* **realtimedata.py:** A Dash application that fetches data from the consumer API and renders live-updating charts.

These components enable end-to-end streaming: data is produced into Kafka by the ETL pipeline, consumed and served by the FastAPI service, and visualized in real time via Dash.
...
│   ├── consumer_service.py      # Kafka consumer FastAPI service.  
│   └── realtimedata.py          # Dash app for real-time metrics.
│
├── src/                         
...
Ensure that the **.env** file is present at the project root and contains the necessary configuration variables:
...
POSTGRES_DB=
GITHUB_TOKEN=
KAFKA_TOPIC=DevSurveyKafkaTopic
KAFKA_SERVER=localhost:9092
```

### 3. Launch the Docker Environment

Run the following command to start all services with Docker Compose, including Kafka, Zookeeper, Airflow, PostgreSQL, the Kafka consumer service, and the real-time dashboard:
...
* Launch the **Apache Airflow** container to orchestrate your ETL DAGs.
* Bring up the **PostgreSQL** container for data storage.
* Run the **FastAPI Kafka Consumer Service** (`consumer_service.py`) exposing `/data` and `/health` endpoints.
* Start the **Dash** application (`realtimedata.py`) for live metric visualization.

### 4. Access the Services

* **Airflow UI:** [http://localhost:8080](http://localhost:8080)
* **FastAPI Consumer API:** [http://localhost:8000/data](http://localhost:8000/data) and [http://localhost:8000/health](http://localhost:8000/health)
* **Dash Real-time Dashboard:** [http://localhost:8050](http://localhost:8050)
...
3. Enter your server name (e.g., localhost) and your database name (e.g., dev\_survey\_insights).
...
1. **Complete Containerization Simplifies Deployment:**
   Using Docker exclusively allows us to quickly bring up the entire environment (Airflow, PostgreSQL, Kafka, consumer, and dashboard) without worrying about machine-specific dependencies.
...
4. **Real-Time Streaming and Visualization:**
   Integrating Kafka with a FastAPI consumer and Dash dashboard provides end-to-end streaming analytics, enabling stakeholders to monitor survey metrics as they arrive.
